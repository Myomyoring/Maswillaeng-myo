...
const UserProfile = ({ member, token, visitUser }) => {
  ...
  useEffect(() => {
    ...
    const getLoginMember = async () => {
      ...
      //   const nick = follower.data.map((item) => {
    ...
    getLoginMember().then((user) => setLoginUserState(user)); // email, nickname, userImage, introduction
  ...
  const { nickname, userImage, introduction, phoneNumber, password, newPwd, RnewPwd } =
    profileUpdateForm;
    ...
    <>
      <div className=" w-1/3 h-screen">
        <div className="w-full text-center">
          ...
          <div className="my-6 text-3xl font-bold">{member.nickname}</div>
          ...
          <div className="m-auto mt-10 ">{member.introduction}</div>
          ...
          {/* {loginUserState?.nickname === visitUser ? (
            <>
              <div className="mt-10 text-sm font-bold text-gray-400">
                ...
                <button className="mx-3" onClick={deleteHandler}>
                  회원 탈퇴
                </button>
              </div>
            </>
          ...
          {loginUserState.nickname === visitUser ? null : (
            <button
              className={
                !followState
                  ? 'm-7 w-10/12 h-10 text-lg text-white font-bold bg-[#EA4E4E] rounded-md mx-3'
                  : 'm-7 w-10/12 h-10 text-lg text-gray-400 font-bold bg-gray-200 rounded-md mx-3'
              }
              onClick={onFollow}
            >
